...
export function ReportForm({ onComplete }: ReportFormProps) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.title && data.description && data.specificType) {
        setFormData((prev) => ({
          ...prev,
          title: data.title,
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="relative group">
        ...
        <label
          ...
          className="block w-full p-8 border-2 border-dashed border-zinc-700 rounded-2xl hover:border-sky-500/50 hover:bg-sky-500/5 transition-all duration-200 cursor-pointer text-center"
        >
          {image ? (
      ...
      <button
        ...
        className="w-full relative group overflow-hidden rounded-xl bg-gradient-to-br from-sky-500 to-blue-600 px-4 py-3.5 text-sm font-medium text-white shadow-lg transition-all duration-200 hover:from-sky-400 hover:to-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
      >
